...
| `debug/a-main/benchmark/cases/bookinfo-ratings-benign-001.json` | **rev-2.1 (2026-07-09), SPECIFIED design-target** — stratum 2 NEGATIVE benign trap: Bookinfo reviews→ratings designed degradation (istio PR #15489). REVIEW-CORPUS R4: `naive_span_error=flag` AND `tracetest_presence=flag` **both FP** (structural error/absence-under-2xx); the ONLY separating column is `mist_trace_shape=no_flag` via a SEMANTIC discriminator (ratings contractually-optional, no required durable effect; entry response communicates degradation). `readback=none-durable → mist_readback=not_applicable`. Precision advantage = pre-registered claim on live capture, not a pilot result. Paired TP twin = sockshop-shipping. Not deployed |
| `debug/a-main/benchmark/cases/sockshop-shipping-swallowed-enqueue-001.json` | **rev-2 (2026-07-09), SPECIFIED** — stratum 1 NATURAL POSITIVE (source-grounded): Sock Shop shipping swallows a RabbitMQ enqueue failure and still acks ("Accepting anyway. Don't do this for real!") while the order is 2xx-acked with shipment set at creation → shipping task silently lost; found by prereg cold-reviewer B (INFO-1). `write_shape=partial-aggregate` (order lands, task lost); status/schema/body no_flag, `naive_span_error`+`tracetest_presence`+`trace_shape`=flag (REQUIRE the G3 two-part tracing mitigation), `readback=none-durable → mist_readback=not_applicable` BY DESIGN — documents the read-back oracle's applicability boundary. The TP twin of bookinfo-ratings-benign |
