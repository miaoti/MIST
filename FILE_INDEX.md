...
| Path | Description |
|------|-------------|
| `debug/a-main/benchmark/README.md` | Main-track prep (no tool code): structure + schema for C2. **SUPERSEDED BY THE STEP-1 RE-FREEZE (banner added 2026-07-08): authoritative pre-registration = `c2c3/c2-freeze.md` rev 2.** §9 = the field-by-field v0.1.0→rev-2 migration map (mechanism split, typed readback, new ack/trace/write-shape fields, oracle rename) for the step-2 seed-case port; §8 scale plan refuted by the depth survey (superseded by the freeze's honest recount) |
...
| `debug/a-main/benchmark/cases/bookinfo-ratings-benign-001.json` | **rev-2 (2026-07-09), SPECIFIED** — stratum 2 NEGATIVE benign trap: Bookinfo reviews→ratings designed degradation (istio PR #15489); `naive_span_error=flag` (FP) vs `mist_trace_shape=no_flag` = the A1 precision/FP test; `tracetest_presence=no_flag` (ratings span present); `readback=none-durable → mist_readback=not_applicable`. Paired TP twin = sockshop-shipping. Not deployed (bookinfo scaled to 0) |
...
| `debug/a-main/benchmark/cases/TT-createaccount-agreement-001.json` | **rev-2 (2026-07-09), CAPTURED** — stratum 1 POSITIVE, the **AGREEMENT anchor** (createAccountFaultMode=fabricatedack): body-carrying POST /inside_payment/account acks `{1,"Create Account Success"}` without persisting → acting user ABSENT from /account. Unlike the bodyless cancel GET, a state-clause/contract comparator CAN bind this postcondition → both MIST read-back AND a comparator catch it (fairness/agreement cell). `mist_readback=flag`. Sidecar `captures/tt-s1-createaccount-agreement` |
