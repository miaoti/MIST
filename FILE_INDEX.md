...
| `debug/a-main/g3-comparator-tt/rider2-bindability-survey.md` | Rider-2 §1 analysis product over the ENTIRE frozen TT blind set (81 endpoints @ 15954a8): per-endpoint state-clause bindability dispositions against the closed primitive set, under a disclosed generous (alias-allowed, doubt-against-MIST) + strict convention pair. FRACTION: G 74/80 = 92.5% bind / 6 NC (3 KEY-SHAPE + orderPay TRANSITION + RESPONSE-KEYED + BATCH); S 62/80 = 77.5%. Two-sided reading: comparator strong on plain CRUD (anti-strawman at scale) but the unbindable residue concentrates on payment/compensation flows (orderPay flip invisible; inside-payment/cancel/preserve/rebook in the frozen set's not_covered) = the residue MIST's value-delta covers. The round-2 reviewers' prepared external-validity answer. PRELIMINARY — needs ≥3-cold-review; executable YAML implemented at breadth-run build |
