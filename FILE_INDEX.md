...
| `debug/a-main/benchmark/b4/runners/3a/flagd-toggle.sh` | Wave-3a pin 1-P0 (B-F1): verified flagd toggle — edits the flagd-config CM `demo.flagd.json` defaultVariant, applies, POLLS the OFREP evaluation (temp PF :18016) until the value flips, prints patch→effect latency; `off` additionally verifies semantic JSON equality vs the wave-start frozen reference (gate1-logs/tenancy-window/otel/flagd-frozen.json, sha256 6dc48f65); exits 2 on no-propagation (fallback ladder: flagd-ui API → pod restart, caller decides). Item 3 inherits verbatim |
