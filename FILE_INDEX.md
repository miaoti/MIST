...
| `debug/a-main/OVERVIEW.md` | **Progress report + presentation notes (English)**: status-at-a-glance (done / in-progress / open bet), then a report with quotable worked examples — the cancel→refund defect (real code snippet), the read-back oracle (membership + the 50→130 vs 50→50 value-delta), the "robot referee" 6-primitive baseline and why it misses, gate-by-gate results (Gate-1 PASS, cancel→refund + Sock Shop "Accepting anyway" head-to-heads, breadth 69/80, FP 0/2127 & 0/1200), anticipated-question talking points, and what's being done now/next |
