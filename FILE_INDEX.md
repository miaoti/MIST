...
| `debug/a-main/c2c3/step2-execution-checklist.md` | The forward manifest for steps 1.9→8 (user directive: write everything down BEFORE the UX wave so nothing is forgotten; **single-box default — no cloud node assumed**). Step 1.9 = the UX wave GATING step 2 (design→review→implement W1–W5→demo-run DoD); step 2 deploy wave (TeaStore/OTel-Demo/Boutique + survey verify-at-deploy riders + 26GB/tenancy/post-reboot-RabbitMQ/CRLF runbook); 2.5 instrumentation (TT/SS OTel, TeaStore Kieker branch, Tracetest smoke vs Bookinfo, TraceAnomaly empirical confirmation, contract-invariant arm spike); 2.75 per-SUT enablement (DoD = the NEW user flow; authoring-cost recorded; seed-case migration); 3a S1/S2 population (two-denominator reporting + tell-free tally + in-class-verified F-corpus); 3b E1 two-tier; 4 M-yield; 5 M-prevalence+S3 (**USER-GATED: rater channel + IRB**); 6 E2 five arms; 7 E5; 8 E6 + sampled-reproduction review. Standing constraints footer. **Rev-3 state: 1.9.2–1.9.5 ✔ DONE (implementation 1d6ed9b, DoD passed, pin stamped); 1.9.6 channel DECIDED by user (in-group MIST-blind grad students) — remaining: rater-materials 3-review + IRB; §2.6 gained the live TT-redeploy runbook (image 1.0.2→1.0.0, scaled-to-0 helm infra, nacos both-members + doubleWrite-off). Rater-materials 3-cold-review DONE (2026-07-09 → rev 3, `REVIEW-RATER-RECONCILIATION.md`); 1.9.6 remaining = IRB(before-contact) + compensation(U1) + §11/§9 screens; step-5 gate gained a 9th check (screening+debrief records, worked-examples authored) + conservative-tie-break-primary adjudication** |
...
| Path | Description |
|------|-------------|
| `debug/a-main/benchmark/README.md` | Main-track prep (no tool code): C2 structure + schema, **REFRESHED to the 11-case rev-2 pilot (2026-07-09)**. Front matter = the SEED/pilot disclosure (6 pos/5 neg; only 2 captured discriminating positives; FP/TP pair + breadth + S3 = unrun scale plan; specified `oracle_expectation` = design target not result). §2 the 11-case table; §3 strata (rev-2.1 R1); §4 anti-circularity + capture_status-keyed evaluability; §5 the FP/TP discriminator (R4) + the agreement→g3 relocation (R5); §7 construct-bias caveat + missing idempotent-no-op control (R6); §8 scale plan (old "3×7≈100–140" arithmetic refuted → freeze §5 recount); §9 migration DONE. Authoritative pre-registration remains `c2c3/c2-freeze.md` rev 2/rev-2.1 |
