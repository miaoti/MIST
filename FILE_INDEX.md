...
| `debug/a-main/g2-comparator/comparator-runner-design.md` | G2 comparator-runner design SPEC (test-first, 2026-07-02): mechanical per-clause translation of the frozen prose contracts into a closed primitive set (HTTP_STATUS/ENVELOPE_STATUS/ENVELOPE_DATA/MSG_CONTAINS/STATE_GET; UNKNOWN→NOT_CHECKABLE, bindings frozen by commit BEFORE any run, binding errors = comparator-infra-failure never a win); flag-gated additive CLI path (mst.comparator.enabled); clear→control(validate bindings)→inject→fault→clear sequence reusing the B1 injector + F2 lessons; pre-stated falsifiable calibration outcome (response clauses PASS under masking, STATE_GET clauses FLAG); 6 pinned tests; G3 head-to-head items out of scope |
