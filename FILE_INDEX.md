...
| `debug/a-main/g3-comparator-tt/runs/` | Persisted head-to-head run logs (prefunded-run*.log + prefunded-reps.txt for the two cancel cells with the value-delta probe balances; agreement-run1.log + agreement-reps.txt for the agreement anchor) — machine-readable evidence, addressing review B's "persist run records" |
...
| `debug/a-main/g3-comparator-tt/REVIEW-HEADTOHEAD-RECONCILIATION.md` | Round-1 ≥3-cold-review reconciliation: all three (A/B/C) ACCEPT-WITH-FIXES, core sound + no false-FIRE; A+B converged on the membership-degeneracy MAJOR (fixed via pre-fund). Disposition table for all 12 findings (FIXED/DISCLOSED/BUILD-NEXT); re-review after the agreement anchor |
