...
| `debug/a-main/c2c3/e2-discrimination-plan.md` | **rev 2 — E2 read-back capability + provenance-closure plan (PENDING re-review; rev 1 got 1 REJECT + 2 ACCEPT-WITH-FIXES, folded).** Run the existing `CancelRefundHeadToHead` (constructed/fabricated-ack stratum) on a RE-INSTRUMENTED traced TT deploy → flip the flagship cell from manual-curl-pre-registered to harness-run-backed + a 3-config trace comparator (naive MISS / service-map-presence MISS / **DB-span-presence CATCH**) → the specification-locality claim (read-back out-of-the-box vs trace needs a pre-specified DB-write assertion). NOT the paper headline (that's the S3 natural discriminator, deferred). Discloses the required changes: traceparent capture (C1), DB-span selector in trace_score.py (C2), 5 fresh-JVM constructed-only runs (C3), P0 re-instrumentation phase |
