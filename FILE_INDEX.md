...
| `debug/a-main/c2c3/wave-275a-plan.md` | **DRAFT rev 1 (for 3-cold-review; the FIRST MIST-tool-code plan after the user opened the gate 2026-07-10 = "MIST 启用+跑测").** Wave 2.75-A = MIST read-back modality bindings + new-SUT observe-mode runs: adds a `ReadbackProbe` transport seam to `DataIntegrityRuntime` (today HTTP-JSON only) so OTel `sql-probe` (psql on `accounting."order"`) and TeaStore HTML `api-get` (`/profile` table) flip `mist_bindable` false→true; per-SUT enablement (authored OpenAPI + registry + triples + auth, code-per-SUT like g3); observe/paired runs on `oteldemo-checkout-lost-001` + `teastore-order-maintenance-masked-001` (+ controls) → pre-registered FLAG-on-fault / no_flag-on-control, converting the two T9-boundary cells into measured MIST results. OTel = derisking primary (clean deterministic psql read-back), TeaStore = fast-follow. §6 has 5 open questions for reviewers (SQL transport exec-vs-JDBC; scope cut; the mist_bindable-flip bar; bespoke-vs-config enablement; observe-vs-paired). Regression guard: the HttpJson probe extraction is behavior-preserving → TT/SS suites stay green |
