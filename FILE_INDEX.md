...
| `debug/a-main/benchmark/b4/capture-specs/` + `captures/` | Capture specs + captured sidecars (§1.95.2). specs: `tt-control-adminroute` (clean control), `tt-benign-contacts-dedupe` (BLOCKED — POST 400), **`tt-cancel-refund` (run 3× per faultmode toggle)**. captures (one `sidecar.json` each): `tt-ctl-adminroute-create`; **`tt-s1-cancel-natural` (200 `{1,"error"}`, bal 50 = refund lost, tell-bearing), `tt-s1-cancel-fabricatedack` (200 `{1,"Success."}`, bal 50 = refund lost, no marker), `tt-s2-cancel-clean` (200 `{1,"Success."}`, bal 130 = refund +80 landed, control)** — all B4-validated (no banned-string leak; opaque-id only; deterministic). Full-schema `case.yaml` (label/provenance/oracle_expectation) = follow-on corpus-assembly step |
