...
| `debug/a-main/TOOL-EXECUTION-PLAN.md` | **Step-by-step plan to TOUCH MIST tool code** (B1 opt-in fault-injection mode + B2 differential data-integrity oracle), v5 review-hardened — survived **three independent cold reviews**; the 2nd/3rd each caught a real MAJOR the 1st missed: (a) writer seam `:706-714` is a code-GENERATION site not runtime plumbing → B2 split into a codegen layer (emit read-back GET + per-run X∈read-back into the generated test) and an orchestration layer (mist-cli inject/clear + control/fault pairing + FP-guard); (b) the make-or-break async-FP Gate-1 PASS criterion was untrustworthy (broker-async ⇒ ~100% timeout-gated) → tightened to require an observed-gated async fraction OR an explicit low-confidence disclaimer + pre-registered timeout + FP-vs-timeout curve. Also: P1 flags corrected (B1=`mist.fault.*` system property; B2=`mst.oracle.dataintegrity.enabled` in MstConfig.Oracle — mining flag is NOT in MstConfig); two-mode fire + SUT-flag-injector-first propagated to README §4/§5 + EXECUTION G0/G1a + target-triples; entity-key must be request-derived (S2 returns data:null). Key decision = two-mode fire (pure-differential headline / gated S1 deferred to G3); execution BLOCKED until the user says "yes" |
