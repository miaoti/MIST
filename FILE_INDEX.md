...
| `debug/a-main/c2c3/traced-capture-wave-plan.md` | Pre-registered PLAN (PENDING ≥3-cold-review; no cluster change until reconciled): earn the flagship **N-vs-0 recall row** by re-running 8 fault/control captures on a TRACED TT deploy — OTel javaagent (Java-8 line, pinned at execution) on the 7 involved services via docker-cp-into-kind-node hostPath + JAVA_TOOL_OPTIONS, OTLP → jaeger-collector.istio-system:4317/4318 (verified live); per-leg trace export via Jaeger HTTP API → `provenance.fault_trace/control_trace`; committed scoring script (naive_span_error / tracetest_presence; `mist_trace_shape` NEVER hand-simulated — deferred unless MIST's real oracle runs); §5 risk table (Java-8 agent canary-first, JTO/-Xmx200m note, ribbon linkage fallback, two-sub-batch RAM mitigation); full restore to today's state; probe-first runbook rule inherited |
