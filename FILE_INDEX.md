...
| `debug/a-main/c2c3/wave-3a-plan.md` | **DRAFT rev 1 (pre-review — execution gated on unanimous 3-cold-reviewer ACCEPT)**: the live-tenant 3a tranche on the standing OTel+TeaStore tenants — (1) OTel `cartFailure` S1+control (the FIRST non-TT `mist_bindable=true` read-back: residual cart via GET /api/cart JSON; partial-aggregate), (2) TeaStore mesh-sever S1+control (T15-verified mechanics; deploy-shape parity pin; mechanism floor datum), (3) `kafkaQueueProblems` pending-vs-missing S2 (probe-gated; disclosed presence-column self-test). Out of scope: Boutique, S3, MIST runs, TT revival. Inherited discipline §5; collapse order 1→2→3; expected counts 18→23 (10 pos/13 neg) |
