...
| `debug/a-main/g3-comparator-tt/g3-headtohead-results.md` | LIVE results of the cancel→refund head-to-head (MIST B2 value-delta vs frozen response-assertion comparator). CONSTRUCTED cell (fabricated-ack drawback) run #1 = MIST FIRE (acked-but-lost refund via /account value-delta) vs comparator MISSED (clean {1,"Success."} passes every bindable check) — the clean win. Natural + agreement cells PENDING. Records the nacos gRPC 1.X-mode reproducibility caveat + fix (rollout restart statefulset/nacos). PRELIMINARY, not yet reviewer-accepted |
