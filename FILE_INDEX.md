...
| `debug/a-main/TOOL-EXECUTION-PLAN.md` | **Step-by-step plan to TOUCH MIST tool code** (B1 opt-in fault-injection mode + B2 differential data-integrity oracle), v4 review-hardened against README §3/§4 + all three Round-2 reviewers (R1/R2/R3) + engineering/consistency passes + an **independent cold-reviewer subagent** (confirmed no MAJOR, spot-verified all 5 cited seams real & line-accurate); key decision = **two-mode fire** (pure-differential headline covers S2/skip-persist that no downstream-error-keyed oracle sees — **propagated to README §4 + EXECUTION G1b** so implementers don't build the S2-blind single rule; gated S1 mode deferred to G3, only ever validated on the Toxiproxy path); soundness adds trace-driven quiescence + gate-coverage, pending-vs-missing (build-now/validate-G3), broker-async FP probe (P3); SUT-flag injector = ground-truth scaffolding not a tool dependency; execution BLOCKED until the user says "yes" |
