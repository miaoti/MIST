...
| `debug/a-main/c2c3/c2-freeze.md` | Step-1 C2 FREEZE (plan §2.4-1; FROZEN ON COMMIT, amendments logged in §6). §1 frozen claim string ("per-case **label-provenance** taxonomy", one word hardened) + first-use definition (by-injection/by-docs/by-adjudication) + proactive-citation + positioning obligations from the sweep. §2 per-case YAML schema, one `cases/<id>.yaml` per case, all 7 review-B-M5 fields tagged (negative control, health preconditions, oracle-config provenance + ONE study-wide frozen MIST commit, label-version-validity bound to image digest, per-case license, trace-visibility class) + machine-checkable schema invariants (S1⇒genuine/by-injection/control-present; underspecified⇒excluded from primary precision denom; zero labels from MIST's predicate). §3 the §8.5-1 adjudication rubric verbatim (3-way {genuine,benign,underspecified}; admissible=docs/spec/source, inadmissible=runtime/traces/MIST; κ<0.6→≤2 calibration-only iterations→full relabel). §4 machine index layout + the frozen scoring contract (fault leg FIRE / control+benign leg NO_FIRE / underspecified excluded / NOT_EVALUABLE its own bucket) + required aggregation views. §5 frozen acceptance floors (≥80+wild; S1≥45/S2≥35/S3 target 20; diversity minima; F-corpus ≥6). §6 amendments log (baseline empty) |
