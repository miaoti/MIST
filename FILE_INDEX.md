...
| `debug/a-main/benchmark/cases/TT-adminroute-lostwrite-001.json` | **rev-2 (2026-07-09), SPECIFIED** — stratum 1 POSITIVE: adminroute SKIPS the cross-service persist to ts-route-service (flag MIST_FAULT_LOSTWRITE_ENABLED @5c471dd8). `trace_visibility=span-presence-visible`: no error span (naive-span/trace-shape clean) but the missing downstream persist span → `tracetest_presence=flag` (NOT a clean MIST-unique win, unlike fabricated-ack) + `mist_readback=flag`. Fork admin-route image not on the current a1767ab3 deploy → specified breadth case |
...
| `debug/a-main/benchmark/cases/TT-adminbasic-contacts-lostwrite-001.json` | **rev-2 (2026-07-09), SPECIFIED** — stratum 1 POSITIVE: second lost write on a different service (adminbasic addContact SKIPS persist → ts-contacts-service @bbf3d6ae); same span-presence-visible shape (`tracetest_presence=flag`); collection-membership read-back (no per-entity GET). Fork admin-basic image not deployed → specified breadth case (triple B) |
