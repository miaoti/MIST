...
| `debug/a-main/prep/g3-sut2-triples-prereg.md` | G3 pre-specification (DRAFT 2026-07-02, pending cold review) of write-path triples on SUT-2/3 per README §8.5-3: KEY finding — Sock Shop GET /orders is PAGINATED (spec-verified) → R1fix (read-back completeness / per-entity GET) promoted to G3 PREREQUISITE; SUT-2 = Sock Shop (SS-A carts add-item clean membership read-back, SS-C POST /orders dual-write-across-broker depth triple + pre-registered async-read-back QUESTION, SS-B register breadth; archived-repo + Go-services-no-OTel caveats); SUT-3 = spring-petclinic (CRUD breadth, TeaStore fallback — persistence REST confirmed but no gateway JSON API); live-verification checklist before any G3 run |
