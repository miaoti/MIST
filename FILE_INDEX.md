...
| `debug/a-main/TOOL-EXECUTION-PLAN.md` | **Step-by-step plan to TOUCH MIST tool code** (B1 opt-in fault-injection mode + B2 differential data-integrity oracle), v6 — four review rounds incl. round-4's three goal-aligned cold reviewers (2/3 SATISFIED: feasibility+prep and design-logic PASS; novelty NOT — Cast caps the *mechanism*). Fixes folded in: writer seam is codegen not runtime (B2 = codegen layer + mist-cli orchestration layer); async-FP Gate-1 PASS tightened (observed-gated async fraction OR explicit low-conf disclaimer + pre-registered timeout + FP-vs-timeout curve; DoD = sync soundness only); **adminbasic has NO per-entity read-back — factual error fixed → business-key collection membership; adminbasic positive only build-verified (smoke pending)**; control-run rescoped to systemic/env FP only; pending-vs-missing shares quiescence's async failure mode; flags corrected (B1 `mist.fault.*` sysprop, B2 `mst.oracle.dataintegrity.enabled`); **novelty re-anchored — primary A-path = C2 benchmark + C3 prevalence (empirical/benchmark A, Cast-independent) + Gate-3 upside, NOT the Cast-capped mechanism**; execution BLOCKED until the user says "yes" |
