...
| `debug/a-main/g3-sut2-wildhunt-plan.md` | SUT-2 wild-hunt PLAN v1 (DRAFT for ≥3-cold-review; user-directed 2026-07-04 pivot after the executable-breadth reject — plan-first-then-review-then-manifest, same loop as TOOL-PLAN). Target = the Sock Shop `shipping` enqueue-swallow: deployed `weaveworksdemos/shipping:0.4.8` bytecode DEFINITIVELY shows `postShipping` catches any `convertAndSend` exception, logs "Accepting anyway. Don't do this for real!", and returns HTTP 200 + the Shipment unconditionally = natural self-documented ASYNC acked-but-lost (complements TT's SYNC refund-loss). Design crux = the read-back observable: `getShippingById` is a stub, so the write's only durable sink is the RabbitMQ `shipping-task` queue → read depth/publish-count over HTTP (mgmt API :15672 or exporter :9090), scale queue-master→0 for monotonic depth, value-delta probe; surgical fault = sever shipping→rabbitmq AMQP 5672 (Istio/NetworkPolicy) with the broker+read-back API kept UP. Comparator (blind response contract) PASSes on 200+echo → structural MIST win. Honest risks R1-R6 (read-back fairness, observable degeneracy, comparator-contract fairness, fault naturalness, async disclaimer, contribution sufficiency); framing options A(full head-to-head, rec)/B(demo)/C(kill-switch). Success = MIST FIRE+FP0, comparator miss, N-stable, ≥3-review |
