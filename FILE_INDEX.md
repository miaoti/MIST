...
| `debug/a-main/TOOL-EXECUTION-PLAN.md` | **Step-by-step plan to TOUCH MIST tool code** (B1 opt-in fault-injection mode + B2 differential data-integrity oracle), v7 — five review rounds; round-5's three goal-aligned cold reviewers all **OVERALL SATISFIED** (feasibility+prep, novelty+contribution, design-logic; E verified Cast arXiv abstract → deltas accurate + no overclaim, capped mechanism ceiling does NOT block since primary A-path = Cast-independent empirical/benchmark leg). Fixes folded across TOOL-PLAN/README/EXECUTION/benchmark/prep: writer seam is codegen not runtime (B2 = codegen + orchestration layers, state via in-process JUnitCore thread-local); async-FP Gate-1 PASS tightened (observed-gated fraction OR disclaimer, pre-registered timeout + numeric sync-FP bar, DoD = sync soundness only); adminbasic has NO per-entity read-back (business-key collection membership; positive only build-verified, smoke pending — propagated to §4); control-run rescoped to systemic/env FP; benign-trap taxonomy incl. accept-then-drop; pending-vs-missing FN + anti-gaming async blind spot disclosed; novelty re-anchored to empirical/benchmark MAIN-track A (C2 benchmark w/ costed 120–160 scale plan + C3 defect-yield/prevalence, Cast-independent) + Gate-3 upside; Cast comparator pre-committed (blind-authored assertions, matched recall). execution BLOCKED until the user says "yes" |
