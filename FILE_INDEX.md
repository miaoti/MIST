...
| `debug/a-main/TOOL-EXECUTION-PLAN.md` | **Step-by-step plan to TOUCH MIST tool code** (B1 opt-in fault-injection mode + B2 differential data-integrity oracle), v3 review-hardened against README §3/§4 + all three Round-2 reviewers (R1 novelty / R2 / R3) + engineering & consistency passes; evidence-anchored to research/01 seams + the Gate-1 smoke result; key decision = **two-mode fire** (pure-differential headline = per-run metamorphic relation covering S2/skip-persist that no downstream-error-keyed oracle can see — E2-measurable delta, NOT an unverified "Cast can't reach" claim — and keeping read-back trace-independent for de-circularization; gated S1 mode deferred to G3 with Toxiproxy); soundness adds trace-driven quiescence + gate-coverage, pending-vs-missing, broker-async FP probe; SUT-flag injector framed as ground-truth scaffolding not a tool dependency; execution BLOCKED until the user says "yes" |
