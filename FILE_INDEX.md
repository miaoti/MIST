...
| `debug/a-main/g3-comparator-tt/rider2-bindability-survey.md` | Rider-2 §1 analysis product over the ENTIRE frozen TT blind set (81 entries @ 15954a8): per-endpoint state-clause bindability dispositions against the closed primitive set, generous (alias-allowed, doubt-against-MIST) + strict convention pair. POST-REVIEW FRACTION (A+B converged BLOCKING folded in): G 71/80 = 88.75% bind / 9 NC (3 KEY-SHAPE + 3 OBJECT-ABSENCE #12/#23/#52 — absence on single-object per-entity reads is VACUOUS since extractItems returns empty for non-array data + orderPay TRANSITION + RESPONSE-KEYED + BATCH); S 61/80 = 76.25%. Collection-shape rule now explicit (absence→list reads only; per-entity presence→entity-matches). Reading reworded: residue is STRUCTURAL primitive-vocabulary gaps; OBJECT-ABSENCE = exactly the cancel→refund read-back shape; payment/compensation axis = 2 separately-scoped facts (not_covered coverage fact + depth-cell 3/3 NOT_CHECKABLE). Reviewer C pending |
