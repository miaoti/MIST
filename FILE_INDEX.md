...
| `mist-cli/src/main/java/io/mist/cli/fault/PairedFaultExecutor.java` | B1.3 orchestrator (clear→control→inject-all→fault→clear-finally, crash/inject-failure safe) + B2.3 pure-differential verdicts + B2.4 benign FP probe (N flag-off runs, FP rates per stratum, FP-vs-timeout curve, ≤5% bar) and the JSON pairing report. |
...
| `debug/a-main/prep/gate1-result.md` | Main-track Gate-1 validation RESULT (2026-07-02): automated pairing verdict = INCONCLUSIVE / infra-blocked (NOT a PASS, NOT a mechanism-FAIL) — run #2 executed the pairing LIVE (generation → control run → SUT-flag inject via rollout → fault run) but aborted before writing any FP/verdict report: station-pair isolation degraded to pass-through under memory-pressured ts-station-service, and the node wedged during the fault-flag clear so the F2 fail-safe correctly threw before writeReport(). Records what executed, why no report, the correct fail-safe behavior, mechanism still validated only by manual G0, and the ≥32GB-box follow-up to obtain the numbers |
...
| `debug/a-main/prep/g2-novelty-comparator-prereg.md` | G2 pre-registration (DRAFT 2026-07-02, pending cold review): the one-paragraph Cast delta a skeptical PC accepts (concede detection primacy; claim the combination: generation + black-box OTel + label-free read-back + open benchmark, with hygiene rules) + the fair-comparator protocol (Filibuster-style FI + per-endpoint assertions authored BLIND to the fault set and frozen by commit hash; matched budget/recall; miss-category table; drop-SUT-not-weaken rule; decisive-result definition); G2 execution checklist |
| `debug/a-main/prep/g3-sut2-triples-prereg.md` | G3 pre-specification (DRAFT 2026-07-02, pending cold review) of write-path triples on SUT-2/3 per README §8.5-3, grounded in the IN-REPO evaluation/suts/sockshop harness (deploy into bookinfo's kind+Istio+Jaeger cluster; swagger 26 ops/12 writes; generalization already validated zero-edit): KEY findings — Sock Shop orders-svc GET is PAGINATED → R1fix promoted to G3 PREREQUISITE; front-end does NOT propagate W3C → quiescence upgrade shallow as-deployed (mitigation: OTel javaagent on Java legs via JAVA_TOOL_OPTIONS, the TT-proven trick); BFF writes are session-scoped (cookie) → MstAuthHandler extension needed. SUT-2 = Sock Shop (SS-A POST /cart Java-leg headline, SS-C POST /orders dual-write-across-broker depth + pre-registered async-read-back QUESTION, SS-B addresses/cards Go-leg breadth); SUT-3 = spring-petclinic (TeaStore fallback); live-verification checklist keyed to the existing scripts |
