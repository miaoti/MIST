...
| `debug/a-main/benchmark/b4/capture-specs/oteldemo-emptycart-flow.yaml` | Wave-3a item-1 pair spec (leg = flagd `cartFailure`): cart POST + checkout POST + TWO cart-read observations = the effect-directed CART-EMPTIED read-back of record (control `[]` = emptied-state present; fault = the item persists). Header carries the toggle pointer, the A-6 psql isolation note, and the export sharp edge (service=checkout only). Captures `oteldemo-emptycart-{swallowed,control}/` |
