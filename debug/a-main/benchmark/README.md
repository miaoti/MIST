...
> sockshop) vs presence=FLAG ran-and-caught via the linked consumer trace.** **WAVE 2.75-A (2026-07-10): both those T9 read-back boundaries RESOLVED — MIST's read-back oracle was bound (TeaStore JSON `/rest/orders`; OTel `accounting.shipping` psql, key=request-derived street_address C-R2) and RAN paired, FIRE 5/5 each, ground-truth-verified; the 4 cases' `mist_bindable` flipped false→true and `mist_readback`→flag/no_flag, entering the MIST recall denominator. HONESTLY: TeaStore=SOLE-oracle, OTel=PRESENCE-CONCORDANT — NEITHER a discrimination win (that stays the TT fabricated-ack case, still unmeasured by any MIST run). RESULT = `../c2c3/wave-275a-result.md`.** `mist_trace_shape` stays
...
| `teastore-order-maintenance-masked` | 1 | positive | **captured** | NATURAL un-forked masked write: the SUT's own maintenance flag makes the persistence CREATE fabricate 201/`-1` while the confirmed page renders; marker absent in-window + post-restore, DB intact. **All three ack columns ran-and-missed; trace cells n/a (Kieker)**. **WAVE 2.75-A: T9 HTML boundary RESOLVED — MIST read-back bound to the durable JSON `/rest/orders`, `mist_bindable` false→true, `mist_readback`=FLAG (paired FIRE 5/5, ground-truth-verified; SOLE-oracle datum, NOT a discrimination win)**; notes carry the mesh-503 rider (verified-masked, 3a candidate) + T15 plain-VS interception datum |
...
| `oteldemo-checkout-lost` | 1 | positive | **captured** | **the ASYNC FLAGSHIP**: broker-down → 200 ack at ~0.02 s, accounting row PERMANENTLY absent (post-restore + post-verified-heal). MEASURED: naive=no_flag ran-and-missed (zero error spans; producer span PRESENT+CLEAN both legs); presence=FLAG ran-and-caught (linked consumer trace, `presence_scope=file`). **WAVE 2.75-A: T9 psql boundary RESOLVED — MIST read-back bound to `accounting.shipping` (key = request-derived street_address, C-R2), `mist_bindable` false→true, `mist_readback`=FLAG (paired FIRE 5/5, ground-truth-verified STILL-0-post-heal; PRESENCE-CONCORDANT with presence=flag, NOT a discrimination win)**. Notes: recovery-window datum (replaced kafka pod wedges both rdkafka clients) |
