# sockshop — rater documentation bundle (LIGHT)

**Pinned version:** Sock Shop (Weaveworks microservices-demo) — pinned image set.

**Sole source of truth for labelling cases on this system:** the files in THIS bundle —
`sockshop-openapi.yaml` (the pinned API spec) + `BEHAVIOR.md` (per-endpoint durable-contract
