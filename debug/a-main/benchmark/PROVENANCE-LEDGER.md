...
| teastore-order-depdown~~-specified~~ | natural | teastore-db scale-0 (by-docs) | **CAPTURED 2026-07-20 (PVC; N=6/6)** | ❌ refs OrderRepository/NonBalancedCRUDOperations | 201 / `-1L` → confirmed page | clean | **native** (`-1L` chain) |
...
| **F3** ✅RESOLVED 2026-07-20 | `teastore-order-depdown-specified-001` *was* SPECIFIED, not CAPTURED (db had no PVC, so a scale-cycle wiped the absence evidence). **Now CAPTURED live** on a PVC-backed db (a deploy change, not a SUT change), fault absent **N=6/6** under the buffer-drop protocol (`b4/cset/teastore-depdown/depdown-legs.log`). | No longer an evidence-strength outlier — it may be pooled with the captured positives. Corpus is now 12 captured / 0 specified. (Case id still contains "specified"; `mist_readback_oracle` stays `not_applicable` until a MIST run scores it.) |
...
| **F11** ✅RESOLVED 2026-07-20 | ~~Headline "11 positives" folds in 1 never-captured case (F3).~~ F3's case is now captured. | Superseded: the headline is **"12 captured, 0 specified"**; the "captured + specified" split no longer applies. |
...
- **Confirmed unanimously:** F1 (catch is `CancelController`, not `cancelOrder` — A + B both verified
  from source; §4 worked example is line-accurate), F3 (`teastore-depdown` was genuinely
  SPECIFIED-not-captured *at review time*; later captured 2026-07-20 — F3 resolved), the source claims underpinning §4, the machine `status==1`⟺success
...
- **New owed items (C, folded into F9–F14):** labeling-process/κ disclosure (→ C3), a measured-vs-
  aspirational oracle-cell summary, ~~a "10 captured + 1 specified" headline~~ (RESOLVED 2026-07-20 → now "12 captured, 0 specified"), a ~8-site headline, a
