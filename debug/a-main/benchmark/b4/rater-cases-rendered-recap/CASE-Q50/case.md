...
- `t+3118ms`  observed state via `durable route store, scoped by the start/end stations in the request above (single-key lookup, not a windowed list): did the route collection gain the submitted route?`: HTTP 200 — `route collection count unchanged (the submitted route is not present)`
