# teastore — rater documentation bundle (LIGHT)

**Pinned version:** TeaStore (Descartes) — pinned image set; Kieker-only, no distributed OTel tracing.

**Sole source of truth for labelling cases on this system:** the files in THIS bundle —
`teastore-openapi.yaml` (the pinned API spec) + `BEHAVIOR.md` (per-endpoint durable-contract
