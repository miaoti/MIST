...
| # | Item | Size | Why |
|---|---|---|---|
| E1 | Ablation/scope switches: discovery=llm/lexical/off, extraction=llm/heuristic, affinity-prior on/off, rotation on/off, P0 on/off, **trace-grounding off + pool-narrowing off (the A0′ switches)**. **Pre-step CLOSED 2026-07-16 (read-only verification): NO JIT-bind knob exists — `SemanticDependencyRegistry` is built unconditionally (`MistGenerator.java:349`; no `semantic.*`/`jit.*` key on the MstConfig surface) — and NONE IS NEEDED for internal validity: JIT binding fires only in multi-root scenarios (`:1611`, `isTopLevelRoot && rootIndex > 1`), registers dependency EDGES resolved from in-run prior-root responses (in-sequence machinery, like context-reuse), and is deterministic-from-spec → IDENTICAL across arms; it partitions parameters into dependent/independent upstream of the treatment, so the value-source contrast operates on the same independent-parameter set in every arm. Disposition: constant-by-construction + PROTOCOL disclosure (report the arm-invariant independent-parameter denominator per SUT; JIT-bind disclosed beside context-reuse as propagation-not-sourcing); the per-arm call-band cross-check (§6.1 #13) stands; an optional knob deferred unless paper reviewers demand it. E1 stays S-M.** | S-M | RQ1/RQ2 internal validity (Round-1 B-B1, C-F11) |
