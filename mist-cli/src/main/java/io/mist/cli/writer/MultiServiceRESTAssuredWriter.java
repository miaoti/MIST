                    ...
                    for (MultiServiceTestCase.StepCall step : scenario.getSteps()) {
                        ...
                        if (allureReport) {
                            ...
                            if (!requestBody.isEmpty()) {
                                ...
                                if (__diTriple != null) {
                                    // G3 rider: the generation-time correlator <class>.<method>#<stepIdx>
                                    // is identical across the control and fault legs (the same generated
                                    // file runs twice), so the pairing join aligns by it, not position.
                                    // The class prefix keeps it unique across a run's many generated
                                    // classes (review A-F2 — method names reset per class).
                                    pw.println("                        // Main-track B2: baseline read-back + isolation-key freshening (no-op outside pairing runs)");
                                    pw.println("                        requestBody" + stepIdx + " = io.mist.cli.fault.DataIntegrityRuntime.beforeWrite(\"" + escape(__diStepKey) + "\", \"" + escape(className) + "." + escape(testMethodName) + "#" + stepIdx + "\", requestBody" + stepIdx + ");");
                                }
